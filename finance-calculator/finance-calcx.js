function TaycorCurrencyFormat(inp){inp = parseFloat(inp).toFixed(2);inp += '';x = inp.split('.');x1 = x[0];x2 = x.length > 1 ? '.' + x[1] : '';var rgx = /(\d+)(\d{3})/;while (rgx.test(x1)) {x1 = x1.replace(rgx, '$1' + ',' + '$2');}var ret = x1 + x2;if((''+ret) == 'NaN') return '0.00';return ret;}
